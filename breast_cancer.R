...
# to select our tuning parameter we will do cross validation
set.seed(1)
wide_grid <- 10^seq(-5, 1, length=200)
...
# we can also use adaptive validation to select our tuning parameter for the lasso, 
#  although this will probably be worse for prediction it should be better for support recovery
# we perform adaptive validation as in algorithm 4.4 from fumdamentals from high dimensional statistics
adaptive_validation <- function(loss, lambda_grid, beta_hats, factor) {
  r <- max(lambda_grid)
  ...
  while ((r != min(lambda_grid)) && (!r_av_found)) {
    beta_hat_r <- coef(beta_hats, s=r)
    r_prime <- max(lambda_grid)
    message(sprintf("r: %s\n", r))
    while ((r_prime > r) && (!r_av_found)) {
      ...
      if (loss(beta_hat_r_prime - beta_hat_r) > factor * r_prime + factor * r) {
        r_av_found <- TRUE #exit from both loops 
        
        # before we exit we choose the last r for which the condition was still fulfilled
        r <- min(lambda_grid[lambda_grid > r]) 
        r_av <- r 
      }
      r_prime <- max(lambda_grid[lambda_grid < r_prime])
    }
    r <- max(lambda_grid[lambda_grid < r])
  }
  return (r_av)
}
...
# we use the maximum norm is used for lasso adaptive validation
...
r.av.lmax <- adaptive_validation(l_max, wide_grid, beta_hats, factor)
...
lasso.av.l1.fit <- glmnet(x[train, ], y$target[train], alpha=1, lambda=r.av.l1)
...
# lasso adaptive validation with max norm loss
lasso.av.lmax.pred <- predict(lasso.av.lmax.fit, s=r.av.lmax, newx=x[test, ])
mean((lasso.av.lmax.pred - y$target[test])^2)

# lasso adaptive validation with l1 norm loss
lasso.av.l1.pred <- predict(lasso.av.l1.fit, s=r.av.l1, newx=x[test, ])
...
lasso.av.lmax.predictors <- which(coef(lasso.av.lmax.fit) != 0)[-1]
lasso.av.l1.predictors <- which(coef(lasso.av.l1.fit) != 0)[-1]

lsquares.predictors
ridge.predictors
lasso.cv.predictors
lasso.av.lmax.predictors
lasso.av.l1.predictors

# and the overlapping predictors
Reduce(intersect, list(v1 = lasso.cv.predictors, v2=lasso.av.lmax.predictors))
